"""Write analysis results back to the Excel file for the first 2 stocks."""
import sys
...
col_date = headers["평가일자"]
col_value = headers["투자가치"]
col_bm = headers["BM"]
col_moat = headers["해자"]
...
for row_idx in range(2, ws.max_row + 1):
    ...
    if ticker in results:
        r = results[ticker]
        ws.cell(row=row_idx, column=col_date).value = r["평가일자"]
        ws.cell(row=row_idx, column=col_value).value = r["투자가치"]
        ws.cell(row=row_idx, column=col_bm).value = r["BM"]
        ws.cell(row=row_idx, column=col_moat).value = r["해자"]
        updated += 1
        print(f"  Updated row {row_idx}: {ticker} ({ws.cell(row=row_idx, column=2).value})")

wb.save(XLSX_PATH)
print(f"\nDone: {updated} stocks updated in {XLSX_PATH}")
