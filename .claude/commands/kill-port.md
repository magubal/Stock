...
4. `diagnose` 인자면 진단 모드 (예: `/kill-port diagnose 8000`)
...
- **PID 실존 검증**: `tasklist`로 프로세스가 실제 존재하는지 확인
- **2단계 종료**: 부드럽게(`taskkill /T`) → 실패 시 강제(`taskkill /F /T`)
- **Ghost 감지**: PID가 없는데 LISTENING이 남는 유령 프로세스 자동 분류
- ghost/stuck 시 `portproxy`, `http servicestate`, `sc query http` 자동 진단
...
- 유령 프로세스(ghost listener) 원인 진단이 필요할 때
